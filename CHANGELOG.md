...
- Windows: **입력 불가한 입력창**(비활성·읽기 전용)에는 배지를 표시하지 않는다.
  ...
  `IsEnabled`·ValuePattern `IsReadOnly`로 판별한다.
- Windows: 카렛이 포커스를 가진 뒤 **다른 곳을 클릭해 편집 불가 영역으로 옮겨가면**
  배지를 숨긴다(예: 웹 페이지의 읽기 전용 본문) — 위 입력 불가·읽기 전용 필터로
  걸러진다. (같은 창의 포커스를 바꾸지 않는 비포커스 영역 클릭은 입력 훅 없이
  감지할 수 없어 카렛이 그대로 포커스를 유지하면 계속 표시된다.)
...
- Windows: 크롬·Electron 등 UIA 폴백 경로에서 그 앱을 떠나 다른 창으로 전환해도
  ...
  전역이라 포커스 상실을 못 봤다. 포커스 요소가 **포그라운드 프로세스**에 속할
  때만 표시하도록 수정(카렛이 포커스를 잃으면 숨김).
- Windows: 설정 창에서 배지 불투명도를 조절해도 반영되지 않던 문제 — 알파를 시작
  시 한 번만 읽었다. 데몬이 약 2초마다 설정을 다시 읽어 **재시작 없이** 반영한다.

### Changed
- Windows 성능: 배지가 숨김(카렛 없음) 상태일 때 폴링을 절반 주기(≈300ms)로
  낮춰 유휴 시 IME 질의·UIA 조회 비용을 줄인다(표시 중엔 150ms 유지). 카렛이
  멈춰 있으면 `SetWindowPos` 호출도 생략.

## [0.8.6] - 2026-07-24

### Fixed
- Windows 멀티모니터: 보조 모니터(주 화면 왼쪽/위쪽은 좌표가 음수)의 카렛에서
  배지가 따라가지 못하고 주 화면 가장자리에 붙던 문제 — 위치를 `0`으로 자르지
  않고 **카렛이 있는 모니터 경계**로 클램프하도록 수정.

### Added
- Windows: 크롬·Electron 등 **자체 카렛을 그려 Win32 카렛이 없는 앱**에서도
  배지 표시. Win32 카렛(`GUITHREADINFO`)을 먼저 시도하고, 없으면 UI Automation
  TextPattern의 선택 영역 사각형으로 폴백한다(편집/문서 컨트롤에 한정).
...
### Changed
- 배지를 **마우스 위치와 무관하게** 포커스된 입력 카렛 옆에 표시한다.
  이전에는 마우스가 그 입력 창 위에 있을 때만 떴다. 이제 macOS는 포커스
  요소의 카렛을, Windows는 타이머로 포커스 창의 카렛을 따라간다(입력 훅
  없이 WM_TIMER 폴링). 카렛을 못 읽으면 숨긴다.
...
- macOS 멀티모니터: 보조 모니터의 입력 카렛에 대해 배지가 주 화면에 잘못
  ...
  화면이 아니라 **주 화면(원점 0,0) 높이** 기준으로 하도록 수정.
