...
haneng is a **cross-platform Korean/English IME state indicator**: hover the mouse over a text input and a badge appears near the input showing the current mode — blue **한** (Korean) / gray **a** (English lowercase) / orange **A** (English + Caps Lock). Windows, macOS, and Linux (X11). That is the whole product as of v0.4.0. (The mouse hover is the *trigger*; the badge is *positioned* at the text caret when readable, else falls back to the mouse cursor. Linux is always mouse-relative — X11 has no caret-position API.)
